...
def BensDeconv2d(in_var, var_dict, name="deconv2d"):
    #deconv = tf.nn.conv2u_transpose( in_var, W, output_shape=output_shape, strides=[1, d_h, d_w, 1] )
    deconv = tf.nn.conv2d_transpose( in_var, var_dict["weights"], 
                            output_shape=output_shape, strides=[1, d_h, d_w, 1] )
    deconv = tf.reshape( tf.nn.bias_add( deconv, b), deconv.get_shape() )
    return deconv
...
true_images = tf.placeholder( tf.float32, shape = [None,28], name ="true_images" )
sampled_zs = tf.placeholder( tf.float32, shape = [None,z_dim], name ="sampled_zs" )
#true_images = tf.placeholder( tf.float32, shape = [None,128], name ="true_images" )
#sampled_zs = tf.placeholder( tf.float32, shape = [None,10], name ="sampled_zs" )
'''
variables_dict = {
...
#  show some results
#
#turn on later once at home base:
'''
